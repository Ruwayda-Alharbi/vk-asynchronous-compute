...
// GLFW may dispatch window position/size events after calling glfwSetWindowPos()/glfwSetWindowSize().
...
static void ImGui_ImplGlfw_SetWindowPos(ImGuiViewport* viewport, ImVec2 position)
{
    ImGuiViewportDataGlfw* data = (ImGuiViewportDataGlfw*)viewport->PlatformUserData;
    data->IgnoreWindowPosEventFrame = ImGui::GetFrameCount();
    glfwSetWindowPos(data->Window, (int)position.x, (int)position.y);
}
...
#endif
static void ImGui_ImplWin32_SetImeInputPos(ImGuiViewport* viewport, ImVec2 position)
{
    COMPOSITIONFORM cf = { CFS_FORCE_POSITION, { (LONG)(position.x - viewport->Pos.x), (LONG)(position.y - viewport->Pos.y) }, { 0, 0, 0, 0 } };
