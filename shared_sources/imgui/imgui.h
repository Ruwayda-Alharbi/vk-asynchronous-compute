...
struct ImDrawData;                  // All draw command lists required to render the frame + position/size coordinates to use for the projection matrix.
struct ImDrawList;                  // A single draw command list (generally one per window, conceptually you may see this as a dynamic "mesh" builder)
struct ImDrawListSharedData;        // Data shared among multiple draw lists (typically owned by parent ImGui context, but you may create one yourself)
struct ImDrawListSplitter;          // Helper to split a draw list into different layers which can be drawn into out of order, then flattened back.
struct ImDrawVert;                  // A single vertex (position + uv + col = 20 bytes by default. Override layout with IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT)
...
namespace ImGui
{
    ...
    IMGUI_API void          SetCursorScreenPos(const ImVec2& position);                          // cursor position in absolute screen coordinates (0..io.DisplaySize) or natural OS coordinates when using multiple viewport.
...
struct ImFont
{
    ...
    IMGUI_API void              RenderChar(ImDrawList* draw_list, float size, ImVec2 position, ImU32 col, ImWchar c) const;
...
struct ImGuiPlatformIO
{
    ...
    void    (*Platform_SetWindowPos)(ImGuiViewport* vp, ImVec2 position);        // . . U . .  // Set platform window position (given the upper-left corner of client area)
    ...
    void    (*Platform_SetImeInputPos)(ImGuiViewport* vp, ImVec2 position);      // . F . . .  // (Optional) Set IME (Input Method Editor, e.g. for Asian languages) input position, so text preview appears over the imgui input box. FIXME: The call timing of this is inconsistent because we want to support without multi-viewports.
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_Minimized                = 1 << 7,   // Platform Window: Window is minimized, can skip render. When minimized we tend to avoid using the viewport position/size for clipping window or testing if they are contained in the viewport.
